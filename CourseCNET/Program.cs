    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            //к примеру если без ref обнулить ссылочный тип данных, то с оригинальным ничего не будет.Т.е.обьект метода без ref -копия и создавать ему новую ссылку или разрывать связь мы не можем,
            //а с -точно тот же обьект.Ref ссылается на обьект, т.е.если обьект ссылочный то ref ссылается на ссылку, позволяя ее менять
            //Ссылочные локальные переменые, т.е. ref тип_данных переменная позволяет хранить ссылку на данные и менять их из любого места, но присвоение к таким должно тоже быть с ref
            //Ref не требует изменение передаваемого значения и требует предварительной инициализации
            ...
            */
            ...
            //В версии шарпа 8 интерфейсы могут иметь базовую реализацию, но это нарушает принцип абстракции
            //Интерфейсы можно наследовать и можно наследовать от нескольких
            //Интерфейсы представляют из себя кусок реализации и можно использовать их к примеру в методах для вычленения только нужного куска
            //Интерфейсы полезны для реализации хранения данных, к примеру 3 варианта получения данных реализуют один интерфейс и выдают нужные данные, оч удобно
            //Класс может реализовывать 2 разных интерфейса с одинаковыми методами, но реализовывать их будет один интерфейс, а чтобы было 2 рвзных метода, то нужна явная реализация а она выглядит так - всякие воиды и тп интерфейс.метод() и оба интерфейса не будут иметь модификатор доступа и вызвать эти методы можно только при апкасте до нужного интерфейса и вызова метода его экземпляра
            //Дефолтная реализация в интерфейсах может быть как заглушка если код у всех классов реализующих будет общий, а прописывать это для каждого класса - такое себе
            ...
            // то надо выносить такие методы в интерфейсы 
            ...
            */
            ...
            //Если метод анонимный, то перед его логикой можно написать delegate и он будет воспринят, либо после принимаемого параметра втыкнуть => и логику в фигурных скобках
            ...
            //События не могут быть изменены и инвокнуты вне своего класса, можно только подписываться и отписываться
            ...
            //Расширяемые методы соотвутствуют одному из принципов solid и упрощает подстройку метода под свои нужды
            //Расширяющий метод создается в статическом классе и для определённого типа расширяемого класса пишем: public static void hz(this int l)
            //Такой метод можно вызывать у всех экземпляров этого класса
            //Через запятую в таких методах указываются параметры
            //Чтобы скрыть не всегда нужные методы расширения можно поставить им другой namespace и импортировать его в нужном файле

            //Существуют partial классы и методы
            //Partial тип можно реализовывать в разных файлах
            //Partial используется в случаях когда класс или метод дополняется чем то извне и нужно и сохранить основной функционал и дополнить его генерируемым
            ...
            //В фигурных скобках при создании экземрляра класса можно указать значения для полей а до фигурных можно вызывать конструктор
            //Обьект класса можно создать с помощью его наследника и дефакто это будет класс - наследник но без его методов,
            //но в экземпляре классе - родителя можно с помощью создания ссылки на наследника хранить его данные +те,
            //которые не известны родителю. НО данные можно будет использовать только при явном приведении или использовании
            ...
            //Добавив обьект diogram и перетащив в него классы можно получить граф наследования и там посмотреть наполнение каждого
            ...
            //Добавление слова new к методу оно скрывает родительскую реализацию

            //Throw; из блока catch отправляет ошибку в класс-родитель или можно указать новый эксепшион

            //Singleton - антипаттерн, только если они будет при отсутствии правок, сам решает почти всё и надо заблокировать конструктор
            ...
            //Потоки можно создавать и запускать в них выполнение некоторых методов - new Thread(method).Start()

            //Структуры не наследуются, ток интерфейсы, имеют базовый конструктор, до 10 версии шарпа не могут иметь конструктор без параметров, хранятся в стеке и имеют дефолтное значение, при присвоении копируются, без нуллабле не могут быть нулл, хранятся в стеке, если часть класса, то хранится в куче, быстрый доступ, но для стека нужно, чтобы были небольшими, не подлерживают реализацию базовых значений для полей
            //Классы хранятся в куче, наследуются, нулл по умолчанию, управляются сборщиком мусора, содержат ссылки на данные, а не сами данные, при присвоении копируется ссылка и оба обьекта ссылаются на одни данные, equals по умолчанию сравнивает ссылки
            ...
            //is проверяет является ли обьект экземпляром нужного класса и позволяет потом сделать явное приведение.В новых версиях т.е после 6 он позволяет прямо в условии лобавить имя экземпляра обьекта и работать с ним в блоке
            //Если метод использует родителя в качестве параметра, то чтобы вызывать переопределённый метод наследника обязательно нужны virtual и override
            //New в обьявлении метода нужно только в случае того, что нам не нужно, чтобы вызывалась реализация наследника, а родителя если метод требует родителя
            ...
            //Структуры лучше использовать для частокопируемвх или иммутабельных обьектов, тк они пиздос быстрее, но для передачи в методы без ref структуры с большим колвом полей и методов тяжелее, -
            //а с ref -почти тоже самое как класс по напрягу, т.е для частоиспользуемого обьекта лучше класс. Полиморфизм и наследование не поддерживаемы структурами, поэтому abstract, virtual, protected, override не робят в структурах
            //Но классы удобней для использования в методах без ref, тк не копируются
            //Выделение памяти для структур new structure() просто выставляет дефолтные значения переменнвм
            //~перед конструктором - финалайзер и вызывается при очистке сборщиком мусора и структуры не обладают такой темой
            //Разница в скорости создания обьектов классов и структур заключается в том, что для классов сначала выделяется ссылка, потом область в оперативке
            //При апкасте структуры до к примеру интерфейса к примеру при передаче в метод происходит упаковка,
            //тк интерфейс не значимый тип, а структура значимый и апкастом мы упаковываем
            //Также если мы не переопределили методы обжекта в структуре, то при их вызове так же будет упаковка
            //GetType не virtual, а все значимые типы не имеют своей реализации GetType поэтому при вызове в экземплярах структур этого метода будет упаковка
            ...
            //<T> метод при получении на вход двух параметров обязывает программиста в том, чтобы оба параметра были единого типа, а добавление в этих стрелочках через запятую TValue можно добавить другие типы данных
            //При вызове<T> метода и передачи в него параметров генерится новый метод, ток с переданными типами и этим занимается jit компялиция
            ...
            //Бывают случаи когда T получить неоткуда, тогда при передаче в метод надо сделать так Method<type>()
            ...
            //Array.Empty() не выделяет пространство в управляемой куче, но инициализирует массив и он поддерживает дженерики, за счет чего экономит память
            //Модификатор тип this[int index] - индексатор, который работает как свойство, this используется как название класса, т.е.list[index] this = list
            ...
            //IEnumerable хранит в себе только метод getenumerator, который возвращает обьеки енумератора< T >
            //IEnumerator имеет свойство T Current, bool MoveNext() пока true позволяет итерировать, void Reset() сбрасывает ирератор в ноль
            ...
            //yield позволяет итерировать метод с памятью места остановки и обязывае метод вернуть возвращаемый обьект обернутый в IEnumerable<>
            //yield позволяет возвращать каждый обьект отдельно, а не целый массив, что экономит память, к примеру так можно сгенерировать большое колво обьектов одним методлм
            //Take в LINQ позволяет взять некоторое количество элементов из последовательности
            //К примеру с помощью foreach где после in yield метод с бесконечным циклом генерации чего-то, то можно поставить после него take и сгенерировать нужное колво обьектов,
            //так же он позволяет делать к примеру построчное чтение файла, получение следующего элемента в какой-то ноде к примеру пути
            ...
            //with при создании обьекта и возвращении его где то позволяет поменять ему параметры, к примеру yield returb myobject with { hz}
            //С помощью yield можно считывать построчно данные из бд или файла
            //yield не стоит использовать, когда надо вернуть все элементы сразу и если требуется многократное использование
            //yield позволяет реализовать к примеру фильтр
            //С помощью таймера и yield можно к примеру сделать генерацию до определённого времени или раз в определённое время
            ...
            //машина может сама менять состояние, к примеру принтеру дали задачу, он поменял состояние на рабочее, выполнил работу и поменял свое состояние на спокойное и разблокировал некую логику
            ...
            */
             ...
             * Static friction - трение об обьекти
             ...
             * Лучше держать Update менее пустым, т.е. разносить его внутренюю логику на методы
             ...
             * Каждому параметру можно добавить доп атребут кликнув пкм на него
