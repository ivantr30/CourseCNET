    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            // АСИНХРОННОСТЬ
            // Асинхронность позволяет программе не зацикливаться на одном действие и  -
            // продолжать работать даже после вызова какого то асинхронного метода
            ...
            // task.IsFaulted - не завершилась ли задача
            // tasks.Exception.InnerExceptions - массив ошибок
            // Таски хранят в себе Exception
            ...
            // Ошибка в асинхронщине выкидывается только когда асинхронный метод ждём, т.е.await
            ...
            */
             ...
             */
            //===Linq===
            //Linq - запросы к коллекциям, базам данных, xml и тп
            //Существует немедленное и отложенное выполнение linq запросов
            //IQueryable - интерфейс linq для запросов к удаленным источникам данных
            ...
            //итерация или перебор по выборке, например, в цикле foreach
            //Немедленное выполнение запроса
            //С помощью ряда методов мы можем применить немедленное выполнение запроса. Это методы, -
            //которые возвращают одно атомарное значение или один элемент или данные типов -
            //Array, List и Dictionary.
            //Ну буквально хуйня выше -то, что выполняется немедленно, это к примеру count после -
            //каждой операции, а отложенное -это orderby, тк пробегается по массиву и потом сортирует
            ...
            //Важно использовать либо select либо group чтоб что-то получить
            ...
            //MinBy, MaxBy - возвращает еще и сам объект
            ...
            //Использовать в случаях, когда данные получены или точный неизменный размер -
            ...
            //итерируемая с помощью keyvaluepair, всё уникально и у уникальных ключей уникальные значения
            ...
            //intersect - объеденяет 2 hashset так, что в в том, от которого сей метод вызван -
            //остаются только те значения, которые есть в обоих множествах
